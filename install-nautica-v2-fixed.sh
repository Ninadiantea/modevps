...
echo "================================================"
echo "  NAUTICA PROXY V2 - FIXED INSTALLER"
echo "  (HTTP API + WebSocket Working Together)"
echo "================================================"
echo ""
...
{
  "name": "nautica-proxy-v2-fixed",
  "version": "4.0.0",
  "description": "Nautica Proxy Server V2 - Fixed HTTP API + WebSocket",
  "main": "server.js",
  "scripts": {
  ...
  "keywords": ["proxy", "vless", "trojan", "shadowsocks", "websocket", "http-api"],
  "author": "Nautica Team",
  "license": "MIT"
}
EOF

# Create server.js with FIXED HTTP + WebSocket handling
echo "🔧 Creating server.js with FIXED HTTP + WebSocket..."
cat > server.js << 'EOF'
...
// ===== HTTP MIDDLEWARE (FIRST PRIORITY) =====
app.use(cors());
app.use(express.json());
app.use(express.static('public'));

// Store accounts and proxies
let accounts = [];
let proxyList = [];

// ===== HTTP API ROUTES (ALWAYS WORKING) =====
app.get('/health', (req, res) => {
    res.json({
        service: 'Nautica Proxy Server V2 - Fixed',
        status: 'running',
        domain: DOMAIN,
        port: PORT,
        accounts: accounts.length,
        proxies: proxyList.length,
        features: ['HTTP API', 'WebSocket', 'Proxy Forwarding']
    });
});
...
// ===== WEBSOCKET HANDLER (SECOND PRIORITY) =====
...
// ===== WEBSOCKET UPGRADE HANDLER (AFTER HTTP ROUTES) =====
server.on('upgrade', (request, socket, head) => {
    const pathname = new URL(request.url, `http://${request.headers.host}`).pathname;
    
    // Check if it's a proxy path (WebSocket upgrade)
    const proxyMatch = pathname.match(/^\/(.+[:=-]\d+)$/);
    if (proxyMatch) {
        ...
        // Not a WebSocket path, destroy socket
        socket.destroy();
    }
});
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`✅ HTTP API routes: Active`);
    console.log(`✅ WebSocket handler: Active`);
    console.log(`✅ Protocol separation: Fixed`);
...
<head>
    ...
    <title>Nautica Proxy V2 - Fixed Dashboard</title>
...
<body class="bg-gray-100 min-h-screen">
    <div class="container mx-auto px-4 py-8">
        <div class="bg-white rounded-lg shadow-lg p-6 mb-8">
            <h1 class="text-3xl font-bold text-gray-800 mb-4">🌊 Nautica Proxy V2 - Fixed</h1>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                ...
                <div class="bg-purple-50 p-3 rounded">
                    <span class="font-semibold">Proxies:</span> <span id="proxyCount">Loading...</span>
                </div>
            </div>
        </div>
        ...
        <!-- Proxy List -->
        <div class="bg-white rounded-lg shadow-lg p-6 mb-8">
            <h2 class="text-2xl font-bold text-gray-800 mb-4">🌐 Available Proxies</h2>
            <div id="proxyList" class="space-y-4">
                <p class="text-gray-500">Loading proxies...</p>
            </div>
        </div>
    ...
    <script>
        ...
        async function loadHealth() {
            try {
                ...
                document.getElementById('proxyCount').textContent = data.proxies;
            } catch (error) {
                ...
                <div class="border border-gray-200 rounded-lg p-4">
                    <div class="flex justify-between items-start mb-3">
                        <div>
                            ...
                            <p class="text-sm text-gray-600">Proxy: ${account.proxyName}</p>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <button onclick="copyConfig('${account.id}', 'shadowsocks')" 
                                    class="bg-purple-600 text-white px-2 py-1 rounded text-xs hover:bg-green-700">
                                Copy
                            </button>
                        </div>
                    </div>
...
module.exports = {
  apps: [{
    ...
    log_file: '/var/log/nautica-proxy-v2-fixed.log',
    out_file: '/var/log/nautica-proxy-v2-fixed-out.log',
    error_file: '/var/log/nautica-proxy-v2-fixed-error.log',
...
cat > /etc/nginx/sites-available/nautica-proxy-v2-fixed << EOF
server {
    listen 80;
    server_name $DOMAIN;
...
# Enable site
ln -sf /etc/nginx/sites-available/nautica-proxy-v2-fixed /etc/nginx/sites-enabled/
...
echo "   Logs: pm2 logs nautica-proxy-v2-fixed"
echo "   Restart: pm2 restart nautica-proxy-v2-fixed"
echo ""
echo "✅ FIXES APPLIED:"
echo "   - HTTP API routes working independently"
echo "   - WebSocket upgrade not interfering with HTTP"
echo "   - Proxy loading working properly"
echo "   - Account creation working properly"
echo "   - Protocol separation implemented"
echo ""
echo "🚀 HTTP API + WebSocket working together!"
echo ""
